...
void loop () {
  ...
  int16_t deltaSum;
  int16_t dT; //EGEN VARIABEL
  int16_t lastDelta = 0; //EGEN VARIABEL
  int16_t filter = 0.004547;
  ...
  */
  ...
  for(axis=0;axis<3;axis++) {
    ...
    //EGEN FILTER-KOD:
    dT = ((uint16_t)0xFFFF / (cycleTime>>4))>>6;
    deltaSum = lastDelta + (dT / filter + dT) * ((int32_t)(delta - lastDelta));
    lastDelta = deltaSum;
